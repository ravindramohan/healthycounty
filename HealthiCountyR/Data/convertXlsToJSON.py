...
# Responsible for loading the json file in case it exists
...
def CreateMongoDataBase():
        stateCountyJsonFile = 'StateCountyData.json'
        scrappedStateCountyDataJsonFile = 'ScrappedStateCountyData.json'
        
        # Load the json in case it is already present!
        StateList = LoadJsonData(stateCountyJsonFile)
        scrapedCounties = LoadJsonData(scrappedStateCountyDataJsonFile)

        # Make sure we do not rescrap the data in case we have already done so
        # already!
        if ( StateList == None or scrapedCounties == None or len(StateList) ==  0 or len(scrapedCounties) == 0 ):
            ...
            jsonfile = stateCountyJsonFile
            with open(jsonfile, 'w') as f:
                json.dump(StateList, f, indent = 4)

            jsonfile = scrappedStateCountyDataJsonFile
            with open(jsonfile, 'w') as f:
